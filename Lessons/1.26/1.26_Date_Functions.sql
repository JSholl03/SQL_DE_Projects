...
ORDER BY RANDOM()
LIMIT 10;
SELECT DATE_TRUNC('month', job_posted_date)::DATE AS job_posted_month,
    COUNT(job_id) AS job_count
FROM job_postings_fact
WHERE job_title_short = 'Data Engineer'
    AND EXTRACT(
...
GROUP BY DATE_TRUNC('month', job_posted_date)::DATE
ORDER BY job_posted_month;
SELECT '2026-01-01 00:00:00+00'::TIMESTAMPTZ AT TIME ZONE 'CST';
SELECT job_posted_date AT TIME ZONE 'EST'
FROM job_postings_fact
LIMIT 10;
